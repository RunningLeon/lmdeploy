...
def test_model_convert_internlm_chat_7b(config):
    result, convert_log = convert(config, 'internlm-chat-7b', 'internlm-chat-7b')
...
def test_model_convert_internlm_chat_20b(config):
    result, convert_log = convert(config, 'internlm-chat-20b', 'internlm-chat-20b')
...
def test_model_convert_Baichuan2_7B_Chat_inner_w4(config):
    result, convert_log = convert(config, 'Baichuan2-7B-Chat-inner-w4', 'baichuan2-7b')
...
# model_name: supported models are: dict_keys(['base', 'llama', 'internlm', 'vicuna', 'internlm-chat-7b', 'internlm-chat', 'internlm-chat-7b-8k', 'internlm-chat-20b', 'internlm-20b', 'baichuan-7b', 'baichuan2-7b', 'puyu', 'llama2', 'qwen-7b', 'qwen-14b', 'codellama', 'solar'])
def convert(config, model_case, model_name): 
    ...
    if 'w4' in model_case:
        cmd = ['lmdeploy convert ' + model_name + ' ' + model_path + "/" + model_case + ' --model-format awq --group-size 128 --dst_path ' + dst_path + '/workspace_' + model_case]
    else:  
        cmd = ['lmdeploy convert ' + model_name + ' ' + model_path + "/" + model_case + ' --dst_path ' + dst_path + '/workspace_' + model_case]
    
    convert_log = os.path.join(log_path, 'convert_' + model_case + ".log")

    with open(convert_log, 'w') as f:
        subprocess.run(['pwd'], stdout=f, stderr=f, shell=True, text=True, encoding=True)
        f.writelines('commondLine: ' + ' '.join(cmd) + '\n')

        # convert
        convertRes = subprocess.run(cmd, stdout=f, stderr=f, shell=True, text=True, encoding=True)
        # 命令校验
