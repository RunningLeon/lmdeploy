...
def test_restful_internlm_chat_7b(config):
    ...
    result, start_log, chat_log, restful_log, kill_log = run_all_step(config, model, port)
...
def run_all_step(config, model, port):
    ...
    with allure.step("step2 - command chat regression"):
        chat_result, chat_log = commondLineTest(config, model, TYPE, HTTP_PREFIX + str(port))
        result = result & chat_result

    with allure.step("step4 - restful_test - openai chat"):
        restful_result, restful_log = restfulOpenAiChatTest(config, model, HTTP_PREFIX + str(port))
...
def startRestfulApi(config, model, port): 
    ...
    cmd = ['lmdeploy serve api_server ' + dst_path + '/workspace_' + model + ' --server_name 0.0.0.0 --server_port ' + str(port) + ' --instance_num 32 --tp 1'] 
    ...
    with open(start_log, 'w') as f:
        ...
        # convert
        convertRes = subprocess.Popen(cmd, stdout=f, stderr=f, shell=True, text=True, encoding=True)
        pid = convertRes.pid
