...
def restfulOpenAiChatTest(config, model, url): 
    ...
    for case_detail in sorted(data, key=lambda x: x['order']):
        ...
        messages = []
        for prompt_detail in sorted(case_detail.get('case'), key=lambda x: x['order']):
            new_prompt = {"role": "user", "content": prompt_detail.get('prompt')}
            messages.append(new_prompt)
            file.writelines("prompt:" + prompt_detail.get('prompt') + '\n')

            for output in api_client.chat_completions_v1(model=model_name, messages=messages):
                output_message = output.get('choices')[0].get('message')
                messages.append(output_message)

                file.writelines("output:" + output_message.get('content') + '\n')

                case_result, reason = assert_result(output, prompt_detail.get('assert'))
