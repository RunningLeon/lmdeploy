    ...
    def __init__(self, num_tokens: int, conv_kernel_size: int, state_ids: torch.Tensor, attn_metadata: Any):
        ...
        if self.num_spec_tokens > 0:
            ...
            if not self.is_decoding:
                read_conv_offsets = torch.remainder(self.cache_seqlens[:, None] + 1 + range_idx[None],
                                                    conv_kernel_size + self.num_spec_tokens)
                write_conv_offsets = torch.remainder(attn_metadata.kv_seqlens[:, None] + 1 + range_idx[None],
                                                     conv_kernel_size + self.num_spec_tokens)
