...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def forward(
        ...
        if all_expert_ids is not None:
            all_expert_ids[:, self.layer_idx, :] = topk_ids
        if get_dist_manager().current_context().dist_config.enable_eplb:
...
class Qwen3MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mlp(hidden_states, all_expert_ids=all_expert_ids)
...
class Qwen3MoeForCausalLM(nn.Module, CudaGraphMixin):
    ...
    def __init__(self,
        ...
        self.lm_head = build_rowwise_linear(config.hidden_size,
                                            ...
                                            dtype=dtype,
                                            device=device)
        self.cache_expert_ids = getattr(self.config, 'cache_expert_ids', False)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if all_expert_ids is None:
            return hidden_states
        return dict(hidden_states=hidden_states, all_expert_ids=all_expert_ids)
    ...
    def make_buffers_cudagraph(self, graph_meta: CudaGraphMeta, input_ids, **kwargs):
        ...
        if self.cache_expert_ids:
            input_buffers['all_expert_ids'] = input_ids.new_full(
    ...
    def fill_buffers_cudagraph(self, graph_meta: CudaGraphMeta, **kwargs):
        ...
        if self.cache_expert_ids:
            input_buffers['all_expert_ids'].fill_(-1)
            new_inputs['all_expert_ids'] = input_buffers['all_expert_ids']
        return new_inputs
    ...
    def get_outputs_cudagraph(self, graph_meta: CudaGraphMeta, input_ids: torch.Tensor, **kwargs):
        ...
        if self.cache_expert_ids:
            outputs['all_expert_ids'] = graph_meta.output_buffers['all_expert_ids'][:num_tokens, ...].clone()
        return outputs
    ...
    def prepare_inputs_for_generation(
        ...
        all_expert_ids = None
        if self.cache_expert_ids:
            all_expert_ids = input_ids.new_full(
