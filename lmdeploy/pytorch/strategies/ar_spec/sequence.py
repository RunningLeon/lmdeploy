...
class SchedulerSequenceARSpec(SchedulerSequenceDefault):
    ...
    def _update_token_ids_prefill(self, token_ids: np.ndarray, draft_token_ids: np.ndarray, stop_pos: int = -1):
    ...
    def _update_token_ids_decode(self, token_ids: np.ndarray, draft_token_ids: np.ndarray, stop_pos: int = -1):
        ...
        if stop_pos > -1:
            ...
            # new draft tokens if not stopped
            self.history_cache.append(draft_token_ids)
    ...
    def update_token_ids(self,
        ...
        if routed_experts is not None:
            ...
            if stop_pos > -1:
                routed_experts = routed_experts[:stop_pos+1]
        ...
        if mode == UpdateTokenMode.INPUTS:
            ...
            self._update_token_ids_prefill(token_ids, draft_token_ids, stop_pos=stop_pos)
        else:
            self._update_token_ids_decode(token_ids, draft_token_ids, stop_pos=stop_pos)
