...
def _compact_blocked_fp8_moe_config(num_tokens: int, num_experts: int):
    """Choose a compact MoE config for blocked FP8 weights."""
    if num_experts >= 1024:
        return dict(block_m=64, block_n=128, num_warps=4, num_stages=3)
    if num_tokens <= 1024:
        return dict(block_m=64, block_n=128, num_warps=4, num_stages=4)
    return dict(block_m=64, block_n=128, num_warps=4, num_stages=3)
...
def _should_use_compact_blocked_fp8_moe(input: torch.Tensor, input_scale: torch.Tensor, w1: torch.Tensor,
                                        w1_scale: torch.Tensor, w2: torch.Tensor, w2_scale: torch.Tensor,
                                        topk_ids: torch.Tensor, num_experts: int):
    """Return whether blocked-FP8 MoE projections should use compact scheduling."""
...
def fused_moe_blocked_fp8(input: torch.Tensor,
    ...
    use_compact = _should_use_compact_blocked_fp8_moe(input, input_scale, w1, w1_scale, w2, w2_scale, topk_ids,
                                                      num_experts)
    if use_compact:
        compact_cfg = _compact_blocked_fp8_moe_config(M, num_experts)
